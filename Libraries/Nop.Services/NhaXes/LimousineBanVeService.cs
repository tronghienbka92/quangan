...
namespace Nop.Services.NhaXes
{
    public class LimousineBanVeService : ILimousineBanVeService
    {
        ...
        public virtual List<DoanhThuNhanVienTheoXe> GetAllChuyenTheoNgay(DateTime TuNgay, DateTime DenNgay, string SerchName, int NhaXeId)
        {
            ...
            var query = _chuyendiRepository.Table.Where(m =>m.NhaXeId==NhaXeId &&
                  m.NgayDi < DenNgay
                   && (m.NgayDi >= TuNgay)
                   //&& (m.TrangThaiId == (int)ENTrangThaiXeXuatBen.CHO_XUAT_BEN || m.TrangThaiId == (int)ENTrangThaiXeXuatBen.DANG_DI))
                   &&(m.TrangThaiId != (int)ENTrangThaiXeXuatBen.HUY))
        ...
        public DoanhThuTheoThang GetDoanhThuTheoThang(int ThangId, int NamId, string SearchNam,int XeVanChuyenId)
        {
            var query = _chuyendiRepository.Table.Where(c => c.NgayDi.Month == ThangId
                && c.NgayDi.Year == NamId
                && c.XeVanChuyenId == XeVanChuyenId
                && c.TrangThaiId != (int)ENTrangThaiXeXuatBen.HUY).ToList();
            var arrIds = query.Select(c => c.Id).ToList();
            var doanhthu=new DoanhThuTheoThang();
            var _doanhthu = _datveRepository.Table.Where(c=>(c.TrangThaiId==(int)ENTrangThaiDatVe.DA_XEP_CHO
                || c.TrangThaiId==(int)ENTrangThaiDatVe.DA_DI ) && arrIds.Contains(c.ChuyenDiId.Value)).ToList();
            doanhthu.DoanhThuHopDong = _doanhthu.Sum(c => c.GiaTien);
            doanhthu.SoNgayVanDoanh=query.Select(c => new
                {
                    Ngaydi = c.NgayDi.Date
                }).GroupBy(c => c.Ngaydi).Count();
            return doanhthu;
        ...
        public virtual ListDoanhThuTongHop GetDoanhThuTongHopTheoNgay(DateTime NgayDi, int NhaXeId)
        {
            ...
            item.NgayDi = NgayDi;
            var DN_TKhanhtrinhids = _hanhtrinhRepository.Table.Where(c => c.Id == (int)ENHanhTrinh.DaNang_TamKy).Select(c => c.Id);
            var DN_CLhanhtrinhids = _hanhtrinhRepository.Table.Where(c => c.Id == (int)ENHanhTrinh.DaNang_ChuLai).Select(c => c.Id);
            var TK_DNhanhtrinhids = _hanhtrinhRepository.Table.Where(c => c.Id == (int)ENHanhTrinh.TamKy_DaNang).Select(c => c.Id);
            var CL_DNhanhtrinhids = _hanhtrinhRepository.Table.Where(c => c.Id == (int)ENHanhTrinh.ChuLai_DaNang).Select(c => c.Id);
            var chuyendiDN_TK = query.Where(c => DN_TKhanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();
            var chuyendiTK_DN = query.Where(c => TK_DNhanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();
            var chuyendiDB_CL = query.Where(c => DN_CLhanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();
            var chuyendiCL_DN = query.Where(c => CL_DNhanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();
            //doanh thu da nang tam ky
           item.DTDN_TK = new DoanhThuTongHop();
           var datvesnb = _datveRepository.Table.Where(c => chuyendiDN_TK.Contains(c.ChuyenDiId.Value)
                && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
           item.DTDN_TK.TongLuot = datvesnb.Select(c => c.ChuyenDiId).Distinct().Count();
           item.DTDN_TK.SoKhach = datvesnb.Count();
           item.DTDN_TK.ThanhTien = 0;
           if (datvesnb.Count() > 0)
           {
               item.DTDN_TK.ThanhTien = datvesnb.Sum(c => c.GiaTien);
           }
           //doanh thu  tam ky da nang
           item.DTTK_DN = new DoanhThuTongHop();
           var datvetkdns = _datveRepository.Table.Where(c => chuyendiTK_DN.Contains(c.ChuyenDiId.Value)
                && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
           item.DTTK_DN.TongLuot = datvetkdns.Select(c => c.ChuyenDiId).Distinct().Count();
           item.DTTK_DN.SoKhach = datvetkdns.Count();
           item.DTTK_DN.ThanhTien = 0;
           if (datvetkdns.Count() > 0)
           {
               item.DTTK_DN.ThanhTien = datvetkdns.Sum(c => c.GiaTien);
           }

            //doanh thu da nang chu lai
           item.DTDN_CL = new DoanhThuTongHop();
           var datves = _datveRepository.Table.Where(c => chuyendiDB_CL.Contains(c.ChuyenDiId.Value)
                && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));

           item.DTDN_CL.TongLuot = datves.Select(c => c.ChuyenDiId).Distinct().Count();
           item.DTDN_CL.SoKhach = datves.Count();
           item.DTDN_CL.ThanhTien = 0;
            if(datves.Count()>0)
            {
                item.DTDN_CL.ThanhTien = datves.Sum(c => c.GiaTien);
            }
            //doanh thu chu lai da nang
            item.DTCL_DN = new DoanhThuTongHop();
            var datvecldns = _datveRepository.Table.Where(c => chuyendiCL_DN.Contains(c.ChuyenDiId.Value)
                 && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));

            item.DTCL_DN.TongLuot = datvecldns.Select(c => c.ChuyenDiId).Distinct().Count();
            item.DTCL_DN.SoKhach = datvecldns.Count();
            item.DTCL_DN.ThanhTien = 0;
            if (datvecldns.Count() > 0)
            {
                item.DTCL_DN.ThanhTien = datvecldns.Sum(c => c.GiaTien);
            } 
        ...
        public virtual List<XeDiTrongNgay> GetAllChuyenTheoXeTrongNgay(DateTime NgayDi, string SerchName,int NhaXeId)
        {
            ...
            var query = _chuyendiRepository.Table.Where(m => m.NhaXeId == NhaXeId &&
                  ...
                  && m.NgayDi.Day == NgayDi.Day
                   && (m.TrangThaiId != (int)ENTrangThaiXeXuatBen.HUY)
                   && (m.laixe.HoVaTen.Contains(SerchName) || m.xevanchuyen.BienSo.Contains(SerchName)))
            ...
            foreach (var m in query)
            {
              ...
              //lay toan bo chuyen cua xe trong ngay
               ...
               foreach(var chuyen in _items)
               {
                   ...
                   chuyeninfo.BatDau=chuyen.NgayDiThuc.ToString("dd/MM/yyyy HH:mm");
                   chuyeninfo.KetThuc = chuyen.NgayDiThuc.ToString("dd/MM/yyy") + " " + chuyen.lichtrinh.ThoiGianDen.ToString("HH:mm");
